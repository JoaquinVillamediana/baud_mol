...
<section class="section-catalog">
  <div class="container">
    ...
    <div class="row row-category">
      ...
      @if ($category->countsub < 1) <h3 href="" data-category_id="{{$category->id}}" class="category-name">
        {{$category->name}}<span class="category-arrow"><i class="fas fa-chevron-circle-down"></i></span></h3>
        @else
        <h3 href="" data-category_id="{{$category->id}}" class="category-name">{{$category->name}}<span
            class="category-arrow"><i class="fas fa-chevron-circle-down"></i></span></h3>
        <div id="category-{{$category->id}}" style="display: none" class="col-12">
          @foreach ($aSubCategories as $subcategory)
          @if ($subcategory->category_id == $category->id)
          <h5 data-subcategory_id="{{$subcategory->id}}" class="subcategory-name">{{$subcategory->name}}<span
              class="subcategory-arrow"><i class="fas fa-chevron-circle-down"></i></span></h5>
          ...
          @else
          <div id="subcategory-{{$subcategory->id}}" style="display: none" class="subcategory">
            <h5 class="product">No hay productos disponibles aquí</h5>
          </div>
          @endif
          @endif
          @endforeach
